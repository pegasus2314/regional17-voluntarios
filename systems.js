/* Regional 17 · Sistemas PLERD externos
   Estos accesos abren los sistemas oficiales en una pestaña nueva.
   No se incrustan en iframe para conservar sus sesiones, seguridad y autenticación propias. */
(() => {
  'use strict';
  ...
  const SYSTEMS = [
    {
      id: 'minume-checkin',
      icon: '✓',
      title: 'Check-in MINUME',
      description: 'Control de asistencia y registro de participantes de MINUME.',
      url: 'https://plerd.minerd.gob.do/checkin-minume',
      accent: 'blue'
    },
    {
      id: 'sigel',
      icon: '◎',
      title: 'SIGEL',
      description: 'Sistema de Gestión del Liderazgo para los modelos de centro y distritales.',
      url: 'https://plerd.minerd.gob.do/sigel',
      accent: 'gold'
    }
  ];
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .rv-system-link .rv-system-icon{width:28px;height:28px;display:grid;place-items:center;flex:0 0 28px;border-radius:8px;background:rgba(255,255,255,.08);font-weight:800}
      ...
      .rv-system-link.rv-blue .rv-system-icon{background:rgba(65,132,255,.20);color:#8eb7ff}
      .rv-system-link.rv-gold .rv-system-icon{background:rgba(245,184,55,.18);color:#ffd36a}
      ...
      .rv-systems-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
      .rv-system-card{display:flex;align-items:center;gap:13px;padding:14px;border:1px solid #e4eaf3;border-radius:13px;background:#fff;text-decoration:none;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease}
      .rv-system-card:hover{transform:translateY(-2px);box-shadow:0 10px 24px rgba(7,27,53,.10);border-color:#c9d8ef}
      .rv-system-card .rv-card-icon{width:42px;height:42px;display:grid;place-items:center;flex:0 0 42px;border-radius:12px;font-size:18px;font-weight:900}
      .rv-system-card.rv-blue .rv-card-icon{background:#eaf2ff;color:#1858c7}
      .rv-system-card.rv-gold .rv-card-icon{background:#fff4d8;color:#a66d00}
      ...
      @media(max-width:720px){.rv-systems-grid{grid-template-columns:1fr}.rv-systems-panel{padding:14px}.rv-systems-panel-head{display:block}}
    `;
  ...
  function navHtml() {
    return `<div class="rv-systems-nav" id="${SIDEBAR_ID}">
      ...
      ${SYSTEMS.map(s => `<a class="rv-system-link rv-${s.accent}" href="${s.url}" target="_blank" rel="noopener noreferrer" title="Abrir ${s.title}">
        <span class="rv-system-icon">${s.icon}</span>
        <span class="rv-system-copy"><strong>${s.title}</strong><small>${s.description}</small></span>
      </a>`).join('')}
    </div>`;
  }
  ...
  function ensureDashboardPanel() {
    ...
    panel.innerHTML = `<div class="rv-systems-panel-head"><div><h3>🔗 Sistemas PLERD</h3><p>Accesos rápidos a las plataformas oficiales que complementan la gestión regional.</p></div></div>
      <div class="rv-systems-grid">${SYSTEMS.map(s => `<a class="rv-system-card rv-${s.accent}" href="${s.url}" target="_blank" rel="noopener noreferrer">
        <span class="rv-card-icon">${s.icon}</span><span class="rv-card-copy"><strong>${s.title}</strong><p>${s.description}</p></span><span class="rv-arrow">→</span>
      </a>`).join('')}</div>`;
