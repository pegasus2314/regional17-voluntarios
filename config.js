...
(function(){
  'use strict';
  const css=`
  /* Chat: WhatsApp-style, no list/card appearance */
  ...
  /* Map has been retired from the application */
  ...
  async function openWhatsAppChat(){
    ...
    if(!user){box.innerHTML='<div class="empty">Debes iniciar sesión para usar el chat.</div>';return;}
    const render=async()=>{
      const [mr,pr]=await Promise.all([s.from('chat_messages').select('id,user_id,message,created_at').order('created_at',{ascending:true}).limit(200),s.from('profiles').select('id,full_name')]);
      if(mr.error){box.innerHTML=`<div class="empty"><h3>No se pudo cargar el chat</h3><p>${esc(mr.error.message)}</p></div>`;return;}
      const names=new Map((pr.data||[]).map(p=>[p.id,p.full_name||'Usuario']));
      box.innerHTML=(mr.data||[]).map(m=>{const mine=m.user_id===user.id,name=names.get(m.user_id)||'Usuario',initial=esc(name.trim().charAt(0).toUpperCase()||'U'),time=new Date(m.created_at).toLocaleTimeString('es-DO',{hour:'numeric',minute:'2-digit'});return `<div class="wa-row ${mine?'me':'other'}">${mine?'':`<div class="wa-avatar">${initial}</div>`}<div class="wa-bubble"><span class="wa-name">${esc(name)}</span><p class="wa-text">${esc(m.message)}</p><small class="wa-time">${time}</small></div></div>`}).join('')||'<div class="empty">Todavía no hay mensajes. Sé el primero en escribir.</div>';
    ...
    if(chatChannel)await s.removeChannel(chatChannel);
    chatChannel=s.channel('r17-chat-live').on('postgres_changes',{event:'INSERT',schema:'public',table:'chat_messages'},async()=>{if(document.getElementById('rv-chat-box'))await render()}).subscribe();
    input.focus();
  }

  function installNavigation(){
    const nav=document.querySelector('.sidebar nav');if(!nav)return;
    nav.querySelectorAll('button[data-view="map"]').forEach(b=>b.remove());
    nav.querySelectorAll('button').forEach(b=>{if(b.dataset.r17Chat==='1')return;if((b.textContent||'').trim()==='Chat'){b.remove()}});
    const b=document.createElement('button');b.className='nav-item';b.dataset.r17Chat='1';b.innerHTML='<span>💬</span>Chat';b.onclick=()=>{window.history.replaceState(null,'','#chat');openWhatsAppChat()};nav.appendChild(b);
  }

  const observer=new MutationObserver(()=>{installNavigation();if(location.hash==='#chat'&&document.getElementById('content')&&!document.getElementById('r17-chat-panel'))openWhatsAppChat()});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('load',()=>setTimeout(installNavigation,500));
  setTimeout(installNavigation,1200);
})();