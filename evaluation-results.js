...
(() => {
  ...
  async function load(o,eid){
    ...
    const categoryIds=(cats.data||[]).map(x=>x.id);
    const criteriaRes=categoryIds.length?await db.from('evaluacion_plantilla_criterios').select('id,categoria_id,nombre,max_puntos,orden,activo').in('categoria_id',categoryIds).eq('activo',true).order('orden'):null;
    let criteria=(criteriaRes?.data||[]).slice().sort((a,b)=>Number(a.orden||0)-Number(b.orden||0));
    const official=[['Oratoria',15],['Argumentación',25],['Negociación',20],['Liderazgo',15],['Redacción',25]];
    if(criteria.length===5){criteria=criteria.map((c,i)=>({...c,nombre:official[i][0],max_puntos:official[i][1]}));}
    const asgRes=pids.length?await db.from('evaluacion_asignaciones').select('id,participante_id,estado').in('participante_id',pids):{data:[],error:null};
    if(asgRes.error){content.innerHTML='<div class="rvr-empty">No se pudieron cargar las asignaciones.</div>';return}
    const asgs=asgRes.data||[];const asgIds=asgs.map(x=>x.id);const asgByParticipant={};asgs.forEach(x=>asgByParticipant[x.participante_id]=x);
    const scoreRes=asgIds.length?await db.from('evaluacion_puntuaciones').select('asignacion_id,criterio_id,puntuacion,observacion').in('asignacion_id',asgIds):{data:[],error:null};
    if(scoreRes.error){content.innerHTML='<div class="rvr-empty">No se pudieron cargar las puntuaciones.</div>';return}
    const scoresByAssignment={};(scoreRes.data||[]).forEach(x=>(scoresByAssignment[x.asignacion_id]??={})[x.criterio_id]=Number(x.puntuacion));
    const rows=participants.map(p=>{
      const d=countries[p.delegacion_id];const asg=asgByParticipant[p.id];const raw=scoresByAssignment[asg?.id]||{};
      const vals=criteria.map(c=>raw[c.id]);
      const complete=criteria.length===5&&vals.every(v=>Number.isFinite(v));
      const total=complete?vals.reduce((a,v)=>a+v,0):null;
    ...
    const header=`<tr><th class="col-no">No.</th><th class="col-name">Delegado/a</th><th class="col-country">País</th>${criteria.map(c=>`<th class="col-criterion">${esc(c.nombre)}<br><small>(${Number(c.max_puntos)})</small></th>`).join('')}<th class="col-total">TOTAL<br>(100)</th></tr>`;
    const body=visible.map((x,i)=>{const scoreClass=x.total===null?'score-empty':x.total>=80?'score-high':x.total<60?'score-low':'';return `<tr class="${scoreClass}"><td class="rvr-rank">${i+1}</td><td class="name">${esc(x.nombre)}</td><td class="country">${esc(x.country?.bandera_emoji||'🌎')} ${esc(x.country?.pais||x.delegacion||'—')}</td>${x.vals.map(v=>`<td>${v===undefined?'—':Number(v).toFixed(Number(v)%1?1:0)}</td>`).join('')}<td class="rvr-total">${x.total===null?'Pendiente':Number(x.total).toFixed(2).replace('.',',')}</td></tr>`}).join('');
