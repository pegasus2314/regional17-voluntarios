/* Regional 17 Volunteers — stable calendar + schedule */
(() => {
  'use strict';

  if (window.__R17_CALENDAR_MODULE__) return;
  window.__R17_CALENDAR_MODULE__ = true;

  const esc = v => String(v ?? '').replace(/[&<>\"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const key = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  const monthLabel = d => new Intl.DateTimeFormat('es-DO',{month:'long',year:'numeric'}).format(d);
  const fullDate = d => new Intl.DateTimeFormat('es-DO',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(d);
  ...
  css.id = 'r17-calendar-stable-css';
  css.textContent = `
    .r17-calendar-shell{display:grid;gap:16px}
    .r17-calendar-hero{background:linear-gradient(135deg,#071b35,#174777);border-radius:18px;padding:24px;color:#fff;display:flex;justify-content:space-between;align-items:center;gap:20px}
  ...
  function forDay(d){
    const k=key(d);
    return items.filter(x=>x.fecha===k).sort((a,b)=>(a.hora||'').localeCompare(b.hora||''));
  }

  function chip(x){
    return `<button type="button" class="r17-event-chip ${x._type==='event'?'event':'activity'}" data-r17-cal-item="${esc(x.id)}" data-r17-cal-type="${x._type}">${x.hora?`<small>${esc(x.hora.slice(0,5))} · </small>`:''}${esc(x.nombre||'Sin título')}</button>`;
  }

  function calendarView(){
    return `<div class="r17-calendar-grid"><div class="r17-weekdays"><div>Lun</div><div>Mar</div><div>Mié</div><div>Jue</div><div>Vie</div><div>Sáb</div><div>Dom</div></div><div class="r17-days">${daysInMonth().map(d=>{const same=d.getMonth()===cursor.getMonth(),today=key(d)===key(new Date()),rows=forDay(d);return `<div class="r17-day ${same?'':'muted'} ${today?'today':''}"><div class="r17-day-number"><b>${d.getDate()}</b>${rows.length?`<span>${rows.length}</span>`:''}</div>${rows.slice(0,4).map(chip).join('')}${rows.length>4?`<small>+${rows.length-4} más</small>`:''}</div>`}).join('')}</div></div>`;
  }

  function agendaView(){
    const start=new Date(cursor.getFullYear(),cursor.getMonth(),1),end=new Date(cursor.getFullYear(),cursor.getMonth()+1,0),days=[];
    for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){const copy=new Date(d),rows=forDay(copy);if(rows.length)days.push({d:copy,rows})}
    if(!days.length)return '<div class="r17-empty"><b>🗓</b>No hay actividades programadas este mes.</div>';
    return `<div class="r17-agenda">${days.map(({d,rows})=>`<section class="r17-agenda-day"><header class="r17-agenda-head"><div class="r17-agenda-date"><b>${d.getDate()}</b><span>${d.toLocaleString('es-DO',{weekday:'short'}).replace('.','')}</span></div><div><strong>${d.toLocaleString('es-DO',{weekday:'long'})}</strong><small>${fullDate(d)}</small></div></header>${rows.map(x=>`<div class="r17-agenda-item"><div class="r17-time">${x.hora?esc(x.hora.slice(0,5)):'Todo el día'}</div><i class="r17-dot ${x._type==='event'?'event':''}"></i><div style="flex:1"><strong>${esc(x.nombre||'Sin título')}</strong><p>${esc(x.direccion||'Sin ubicación')}${x.descripcion?` · ${esc(x.descripcion)}`:''}</p></div><span class="pill ${x._type==='event'?'neutral':'success'}">${x._type==='event'?'Evento':'Actividad'}</span></div>`).join('')}</section>`).join('')}</div>`;
  }

  function showDetails(id,type){
    const x=items.find(v=>String(v.id)===String(id)&&v._type===type);if(!x)return;
    const o=document.createElement('div');o.className='overlay';o.innerHTML=`<div class="modal"><div class="modal-head"><h3>${esc(x.nombre||'Detalle')}</h3><button class="icon-btn" data-close>×</button></div><div class="modal-body"><p><b>Fecha:</b> ${esc(x.fecha||'—')}</p><p><b>Hora:</b> ${esc(x.hora?x.hora.slice(0,5):'Todo el día')}</p><p><b>Ubicación:</b> ${esc(x.direccion||'Sin ubicación')}</p>${x.descripcion?`<p><b>Descripción:</b> ${esc(x.descripcion)}</p>`:''}</div></div>`;document.body.appendChild(o);o.querySelector('[data-close]').onclick=()=>o.remove();o.onclick=e=>{if(e.target===o)o.remove()};
  }

  function render(){
    const c=document.getElementById('content');
    if(!c)return;
    c.innerHTML=`<div class="r17-calendar-shell"><div class="r17-calendar-hero"><div><span class="r17-kicker">PLANIFICACIÓN REGIONAL</span><h2>Calendario y cronograma</h2><p>Visualiza actividades y eventos de Regional 17 en una sola agenda.</p></div><div style="font-size:42px">🗓️</div></div><div class="r17-calendar-tools"><div class="r17-month-nav"><button type="button" class="r17-icon-btn" id="r17-prev">‹</button><div class="r17-month-title">${monthLabel(cursor)}</div><button type="button" class="r17-icon-btn" id="r17-next">›</button><button type="button" class="btn small" id="r17-today">Hoy</button></div><div class="r17-tabs"><button type="button" class="r17-tab ${tab==='calendar'?'active':''}" data-r17-tab="calendar">Calendario</button><button type="button" class="r17-tab ${tab==='agenda'?'active':''}" data-r17-tab="agenda">Cronograma</button></div></div><div class="r17-calendar-legend"><span><i></i> Actividades</span><span><i class="event"></i> Eventos</span></div><div id="r17-cal-body">${tab==='agenda'?agendaView():calendarView()}</div></div>`;
    ...
    document.querySelectorAll('[data-r17-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.r17Tab;render()});
    c.querySelectorAll('[data-r17-cal-item]').forEach(b=>b.onclick=()=>showDetails(b.dataset.r17CalItem,b.dataset.r17CalType));
  }
  ...
  async function load(){
    ...
    loading=true;
    const c=document.getElementById('content');
    if(c)c.innerHTML='<div class="r17-empty"><b>🗓</b>Cargando calendario…</div>';
    try{
      const sb=client();
      ...
      render();
    }catch(err){console.error('R17 calendar:',err);if(c)c.innerHTML='<div class="r17-empty"><b>⚠️</b><strong>No se pudo cargar el calendario.</strong><p>La aplicación sigue funcionando. Intenta nuevamente.</p><button type="button" class="btn primary" id="r17-retry">Reintentar</button></div>';document.getElementById('r17-retry')?.addEventListener('click',load)}
  ...
  function activate(){
    ...
    if(!nav)return;
    let b=nav.querySelector('[data-r17-calendar]');
    if(!b){
      b=document.createElement('button');
      b.type='button';
      b.className='nav-item';
      b.dataset.r17Calendar='1';
      b.dataset.view='calendar';
      b.innerHTML='<span>▦</span>Calendario';
      nav.appendChild(b);
    }
    if(!b.dataset.r17CalendarBound){
      b.dataset.r17CalendarBound='1';
      b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();window.__R17_CALENDAR_OPEN__=true;document.querySelectorAll('.nav-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');const h=document.querySelector('.topbar h1');if(h)h.textContent='Calendario y cronograma';load()},true);
    }
    if(window.__R17_CALENDAR_OPEN__ && document.getElementById('content') && !document.querySelector('.r17-calendar-shell') && !loading){load();}
  }

  const observer=new MutationObserver(()=>activate());
  observer.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',activate);
  setTimeout(activate,100);setTimeout(activate,400);setTimeout(activate,1000);setTimeout(activate,2000);
  window.__r17OpenCalendar=()=>{window.__R17_CALENDAR_OPEN__=true;activate();document.querySelector('[data-r17-calendar]')?.click()};
})();
