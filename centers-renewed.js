...
function enhanceCenters(){
 ...
 cards.forEach(card=>{
   ...
   const icon=card.querySelector('.center-icon');
   if(icon){icon.textContent='🏫';icon.setAttribute('aria-hidden','true');icon.classList.add('school-icon');}
   card.classList.add('center-card-institutional');
 ...
 head.innerHTML=`<div><div class="eyebrow">DIRECTORIO REGIONAL</div><h2>🏫 Centros Educativos</h2><p>Directorio regional de centros educativos organizado por distrito.</p></div><div class="centers-total"><strong>${cards.length}</strong><span>centros activos</span></div>`;
 ...
 [...groups.entries()].sort((a,b)=>a[0].localeCompare(b[0],'es')).forEach(([district,items])=>{
   ...
   const title=document.createElement('div');title.className='centers-district-head';title.innerHTML=`<div class="centers-district-title"><span class="district-mark">📍</span><div><h3>${district}</h3><p>Centros educativos registrados</p></div></div><span class="centers-district-count">${items.length} ${items.length===1?'centro':'centros'}</span>`;
