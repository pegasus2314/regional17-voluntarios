(() => {
  'use strict';
  if (window.__R17_RUNTIME_FIX_V5__) return;
  window.__R17_RUNTIME_FIX_V5__ = true;

  const css = document.createElement('style');
  css.id = 'r17-runtime-stability-v5';
  css.textContent = `
  ...
  if(desc?.set&&desc?.get){
    Object.defineProperty(Element.prototype,'innerHTML',{
      ...
      set(value){
        ...
        desc.set.call(this,value);
        if(this.id==='app'){appInitialized=true;queueMicrotask(restoreChat);}
      }
    });
  }
  ...
  window.addEventListener('r17-chat-open',()=>{chatWasOpen=true;preservedChat=document.querySelector('.rv-chat')||preservedChat;});
  window.addEventListener('r17-chat-close',()=>{chatWasOpen=false;preservedChat=null;});

  if(!document.querySelector('script[data-r17-mun-live-sync]')){
    const s=document.createElement('script');
    s.src='./mun-live-sync.js?v=5';
    s.dataset.r17MunLiveSync='1';
